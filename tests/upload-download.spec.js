...
const worksheet = workbook.getWorksheet('Sheet1'); // Get the default worksheet (Sheet1)
    ...
    if (output.row === -1 || output.column === -1) {
        throw new Error(`No se encontro la celda con el texto: ${searchText}`);
    }
...
test('Upload download excel validation', async ({ page }, testInfo) => { // Test case to validate the upload and download of an Excel file testInfo is used to get the path of the downloaded file
    ...
    const download = page.waitForEvent('download'); // Wait for the download event to occur
    await page.getByRole('button', { name: 'Download' }).click(); // Click the download button to download the Excel file

    const dl = await download; // Wait for the download to complete
    const filePath = testInfo.outputPath('download.xlsx'); // Get the path of the downloaded file using testInfo.outputPath() and store it in the filePath variable
    await dl.saveAs(filePath);

    await writeExcelTest(testSearch, updatedValue ,{rowChange:0,colChange:2}, filePath); // se llama a la función writeExcelTest con los parámetros de búsqueda, reemplazo, cambios y ruta del archivo 
    await page.locator('#fileinput').setInputFiles(filePath); // Set the input files for the file input to the specified Excel file
    await page.locator('input.upload').click(); // Click the upload button to upload the Excel file
    const textlocator = page.getByText(testSearch); // Get the locator for the text containing the search text
    const desiredRow = await page.getByRole('row').filter({ has: textlocator}); // Filter the rows to find the one that contains the specified search text
    await expect(desiredRow.locator('#cell-4-undefined')).toContainText(updatedValue); // Assert that the cell in the found row contains the updated value
});
