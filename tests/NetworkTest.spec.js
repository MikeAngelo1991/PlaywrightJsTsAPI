...
const fakePayLoadOrders = { data: [], message: "No Orders" } // variable global para almacenar la respuesta de la solicitud de creación de orden
...
test('@SP Place the order', async ({ page }) => { // se colcoca browser para abrir el navegador
    ...
    // se esta interceptando la respuesta de la solicitud de obtener el conteo del carrito de compras y se esta reemplazando con una respuesta falsa que indica que no hay ordenes
    await page.route("https://rahulshettyacademy.com/api/ecom/order/get-orders-for-customer/*",
        async route => {

            const response = await page.request.fetch(route.request());
            let body = JSON.stringify(fakePayLoadOrders); // se convierte el objeto fakePayLoadOrders a una cadena JSON
            route.fulfill({
                response,
                body,
